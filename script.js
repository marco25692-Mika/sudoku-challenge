...
let friendRanking = JSON.parse(localStorage.getItem('sudoku_friend_ranking')) || [
...
function updatePlayerUI() {
    ...
    const selfEntry = friendRanking.find(f => f.name.includes('(Du)'));
    if (selfEntry) {
        selfEntry.name = `${playerName} (Du)`;
    } else {
        friendRanking.unshift({ name: `${playerName} (Du)`, wins: 0, bestTime: '--:--' });
    }
    
    germanyRanking.forEach(item => {
...
function showGameModeSelectionOverlay(diff) {
    ...
    overlay.innerHTML = `
        <div class="ad-container" style="max-width: 340px; text-align: center; position: relative; padding: 24px;">
            <h3 style="font-size: 1.2rem; font-weight: 700; margin-bottom: 8px;">Modus wählen</h3>
            <p style="font-size: 0.9rem; color: #666; margin-bottom: 20px;">Schwierigkeit: <strong>${diffText}</strong></p>
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <button id="mode-ranking" class="btn btn-primary" style="padding: 14px; border-radius: 12px; font-weight: 600; cursor: pointer;">🏆 Direkt spielen</button>
                <button id="mode-friend" class="btn" style="background-color: #25D366; color: white; padding: 14px; border-radius: 12px; font-weight: 600; cursor: pointer;">💬 Freund herausfordern</button>
                <button id="mode-cancel" class="btn" style="background: #e5e5ea; color: #333; padding: 10px; border-radius: 12px; font-weight: 600; cursor: pointer; margin-top: 5px;">Abbrechen</button>
            </div>
        </div>
    `;
    ...
    overlay.querySelector('#mode-ranking').addEventListener('click', () => {
        overlay.remove();
        startGame();
    });
...
function saveFriendRanking() {
    localStorage.setItem('sudoku_friend_ranking', JSON.stringify(friendRanking));
}

function switchRankingTab(category, btn) {
    currentRankingCategory = category;
    document.querySelectorAll('#tab-ranking .diff-tab').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
...
function updateRankingUI() {
    ...
    let dataset = friendRanking;
    if (currentRankingCategory === 'germany') dataset = germanyRanking;
    if (currentRankingCategory === 'global') dataset = globalRanking;
    ...
    if (currentRankingCategory === 'friends') {
