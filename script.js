...
function runAdFlow(onComplete) {
    ...
    // Erzwinge die Sichtbarkeit und absolute Positionierung direkt oben rechts im Kasten
    ...
    const adInterval = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            xBtn.style.display = 'block'; // Zeigt das X exakt nach Ablauf des Timers an
    ...
    // Da es neu geklont wurde, muss display block nach Ablauf wieder gesetzt werden oder direkt per Event abgefangen:
    // Wir steuern das direkt über einen globalen Klick-Listener auf den Container oder das X:
    ...
    newXBtn.style.display = 'block'; // Vorsichtshalber vorbereiten, wird aber durch den Timer gesteuert
    
    // Wir hängen den Eventlistener sauber an
    ...
    // Timer-basierter Fallback direkt für den Button
    ...
    // Sobald beendet wird, Intervalle aufräumen
    const oldFinish = finishAd;
    // Überschreiben für sauberes Aufräumen
